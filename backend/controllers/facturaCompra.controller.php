...
class FacturaCompraController{
    ...
    public function readOne($facturaCompraID){
        ...
        $facturaCompra = FacturaCompraModel::read("facturaCompra", $facturaCompraID);

        if(empty($facturaCompra)){

            $json=array(
                "status"=>404,
                "detalle"=>"Este factura de compra no existe en la base de datos."
            );
    ...
    public function readAll($nFacturaCompra){

        // Llamando método para hacer la lectura en la tabla de FacturaCompra
        $facturaCompras = FacturaCompraModel::read("facturaCompra", null);

        if(empty($facturaCompras)){

            $json=array(
                "status"=>404,
                "detalle"=>"No hay facturas en la base de datos."
            );
    ...
    public function delete($facturaCompraID){
        ...
        if($delete=="ok"){
            ...
            $json=array(
                "status"=>200,
                "detalle"=>"La factura se eliminó correctamente."
            );
